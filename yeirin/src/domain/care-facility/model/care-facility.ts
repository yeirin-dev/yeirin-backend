 ...
 * 양육시설 생성 Props
 */
export interface CreateCareFacilityProps {
  /** 기관명 */
...
/**
 * 양육시설 복원 Props (DB에서 복원 시)
 */
export interface RestoreCareFacilityProps extends CreateCareFacilityProps {
  id: string;
  isActive: boolean;
  createdAt: Date;
  updatedAt: Date;
...
/**
 * 양육시설 (CareFacility) Aggregate Root
 * - 아동복지법에 따른 양육시설
 * - 양육시설 선생님이 소속되는 기관
 */
export class CareFacility {
   ...
   * 양육시설 생성 (정적 팩토리 메서드)
   */
  static create(props: CreateCareFacilityProps): Result<CareFacility, DomainError> {
    // 대표자명 검증
    ...
    if (props.capacity > 500) {
      return Result.fail(new DomainError('정원은 최대 500명까지 가능합니다'));
    }
   ...
   * DB에서 복원
   */
  static restore(props: RestoreCareFacilityProps): CareFacility {
