...
export class ReviewFixture {
   ...
   * 기본 리뷰 데이터 생성
   */
  static createReviewData(overrides?: Partial<ReviewEntity>): Omit<ReviewEntity, 'id' | 'createdAt' | 'updatedAt' | 'institution'> {
    return {
   ...
   * 여러 리뷰 데이터 생성
   */
  static createMultipleReviewData(count: number, overrides?: Partial<ReviewEntity>): Array<Omit<ReviewEntity, 'id' | 'createdAt' | 'updatedAt' | 'institution'>> {
   ...
   * 기관 엔티티 생성 (리뷰 관계용)
   */
  static createInstitutionEntity(overrides?: Partial<VoucherInstitutionEntity>): VoucherInstitutionEntity {
    const now = new Date();
    return {
   ...
   * 평점별 리뷰 데이터 생성
   */
  static createReviewsByRating(rating: 1 | 2 | 3 | 4 | 5, count: number = 1): Array<Omit<ReviewEntity, 'id' | 'createdAt' | 'updatedAt' | 'institution'>> {
    return Array.from({ length: count }, (_, index) => ({
   ...
   * 도움이 됨 카운트가 높은 리뷰 생성
   */
  static createPopularReview(helpfulCount: number = 100): Omit<ReviewEntity, 'id' | 'createdAt' | 'updatedAt' | 'institution'> {
