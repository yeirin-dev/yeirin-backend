...
import { StartCounselingUseCase } from './start-counseling.usecase';

describe('StartCounselingUseCase', () => {
  let useCase: StartCounselingUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<StartCounselingUseCase>(StartCounselingUseCase);
  });
  ...
  describe('상담 시작', () => {
    ...
    it('존재하지 않는 상담의뢰를 시작하면 NotFoundException을 던진다', async () => {
      // Given
    ...
    it('접수 대기 상태에서 상담을 시작하면 실패한다', async () => {
      // Given
      const requestId = 'request-123';
      ...
      await expect(useCase.execute(requestId)).rejects.toThrow(
        '매칭 완료 상태에서만 상담을 시작할 수 있습니다',
      );
      expect(mockRepository.save).not.toHaveBeenCalled();
    });

    it('이미 진행 중인 상담을 다시 시작하면 실패한다', async () => {
      // Given
      const requestId = 'request-123';
  ...
  describe('응답 DTO 변환', () => {
    it('상담 시작 후 CounselRequestResponseDto로 변환한다', async () => {
      // Given
      const requestId = 'request-123';
      const counselRequest = createMatchedCounselRequest(requestId);
      mockRepository.findById.mockResolvedValue(counselRequest);
